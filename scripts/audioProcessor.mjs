		...
		if (data.mode !== undefined) {
			switch (data.mode) {
				...
				case 'sinfmode':
					...
					this.getValuesVisualizer = (funcValue) => (((Math.sin(funcValue / (128 / Math.PI) + 127) * 128) & 255) + 127);
					break;
				case 'tanfmode':
					this.getValues = (funcValue) => ((Math.tan(funcValue * Math.PI / 128) * 32)) / 127.5 - 1;
					this.getValuesVisualizer = (funcValue) => (((Math.tan(funcValue / (128 / Math.PI) + 127) * 128) & 255) + 127);
					break;
				case 'cosfmode':
					this.getValues = (funcValue) => ((Math.cos(funcValue * Math.PI / 128) * 32)) / 127.5 - 1;
					this.getValuesVisualizer = (funcValue) => (((Math.cos(funcValue / (128 / Math.PI) + 127) * 128) & 255) + 127);
					break;
